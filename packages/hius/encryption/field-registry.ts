...
export type FieldConfig = {
  ...
  hashField?: string;
  // No deterministic-encryption mode exists yet (would need its own cipher
  // mode, not just a schema column). Don't add a `deterministic` flag or a
  // corresponding DB column speculatively — add both together when there's
  // a real cipher mode behind it.
};
