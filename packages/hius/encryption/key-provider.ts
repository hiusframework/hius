...
export interface KeyProvider {
  ...
  // Active key first, then every retired key still accepted for reads.
  // Needed so blind-index search can try every key a value might have been
  // hashed under across a rotation, not just the current one.
  getAllKeys(): KeyBundle[];
}
...
export function createEnvKeyProvider(): KeyProvider {
  ...
  return {
    ...
    getAllKeys: () => [activeKey, ...[...keys.values()].filter((k) => k.keyId !== activeKey.keyId)],
  };
}
...
export function createStaticKeyProvider(
  ...
  return {
    getActiveKey: () => activeKey,
    getKeyById: (keyId: string) => keys.get(keyId) ?? null,
    getAllKeys: () => [activeKey, ...additionalKeys],
  };
}
