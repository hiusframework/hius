...
describe("BlindIndex", () => {
  ...
  test("computeAllCandidates with a single key returns exactly one hash, matching compute()", () => {
    expect(blindIndex.computeAllCandidates("alice@example.com")).toEqual([
      blindIndex.compute("alice@example.com"),
    ]);
  });

  test("after a key rotation, computeAllCandidates still returns the hash under the retired key", () => {
    const oldKey = {
      keyId: "old-key",
      encryptionKey: Buffer.from("c".repeat(32), "utf8"),
      hmacKey: Buffer.from("d".repeat(32), "utf8"),
    };
    const oldBlindIndex = createBlindIndex(createStaticKeyProvider(oldKey));
    const rotatedBlindIndex = createBlindIndex(createStaticKeyProvider(keyBundle, [oldKey]));

    const value = "alice@example.com";
    const hashUnderOldKey = oldBlindIndex.compute(value);
    const candidates = rotatedBlindIndex.computeAllCandidates(value);

    expect(candidates).toContain(hashUnderOldKey);
    expect(candidates[0]).toBe(rotatedBlindIndex.compute(value)); // active key's hash comes first
  });
});
...
describe("rewriteQuery", () => {
  ...
  test("after a key rotation, eq on a searchable field matches rows hashed under either key", () => {
    const registry = makeRegistry();
    const oldKey = {
      keyId: "old-key",
      encryptionKey: Buffer.from("c".repeat(32), "utf8"),
    ...
    const value = "alice@example.com";
    const hashUnderOldKey = oldBlindIndex.compute(value);
    const hashUnderActiveKey = rotatedBlindIndex.compute(value);

    const result = rewriteQuery(eq("email", value), "users", registry, rotatedBlindIndex);

    expect(result).toEqual({
      type: "or",
      conditions: [
        { type: "eq", column: "email_hash", value: hashUnderActiveKey },
        { type: "eq", column: "email_hash", value: hashUnderOldKey },
      ],
    });
  });
});
