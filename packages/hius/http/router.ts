    ...
    for (const route of this.routes) {
      ...
      try {
        ...
        if (err instanceof ValidationError) {
          return unprocessable({ error: err.message, code: err.code, issues: err.issues });
        }
        if (err instanceof NotFoundError) {
          return notFound({ error: err.message, code: err.code });
        }
        if (err instanceof UnauthorizedError) {
          return unauthorized({ error: err.message, code: err.code });
        }
        if (err instanceof ForbiddenError) {
          return forbidden({ error: err.message, code: err.code });
        }
        if (err instanceof ConflictError) {
          return conflict({ error: err.message, code: err.code });
        }
        if (err instanceof UnprocessableError) {
          return unprocessable({ error: err.message, code: err.code });
        }
        if (
          err instanceof SyntaxError ||
          (err instanceof Error && err.message === "Invalid JSON body")
        ) {
          return badRequest({ error: "Invalid JSON body", code: "INVALID_JSON" });
        }
