...
src/test/
  ...
  http/
    ...
    pipeline.test.ts         # Выполнение цепочки пайпов
    routing.test.ts          # DSL defineRoutes, scope, resources, draw, mergeRoutes
    router.test.ts           # Диспетчеризация запросов (метод, params, constraints, pipes)
    validation.test.ts       # validate() + интеграция роутера 422/400
    permit.test.ts           # Фильтрация permit() и permitQuery() по allow-list
```
